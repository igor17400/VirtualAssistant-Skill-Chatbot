...
namespace VirtualAssistantSample.Services
{
    public class BotServices
    {
        ...
        public CognitiveModelSet GetCognitiveModels()
        {
            ...
            //OBS --> Esse codigo possui um erro de System.ArgumentOutOfRangeException por causa do Substring(0, 2), por isso alterei o código
            //para  Substring(0)
            ...
            var cognitiveModel = CognitiveModelSets.ContainsKey(locale)
                ? CognitiveModelSets[locale]
                : CognitiveModelSets.Where(key => key.Key.StartsWith(locale.Substring(0))).FirstOrDefault().Value
                ?? throw new Exception($"There's no matching locale for '{locale}' or its root language '{locale.Substring(0)}'. " +
                                        "Please review your available locales in your cognitivemodels.json file.");
